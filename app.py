...
# HELPERS
# ──────────────────────────────────────────────────────────────────────────────
def show_img(path: str, caption: str = ""):
    if os.path.exists(path):
        st.image(Image.open(path), caption=caption, use_container_width=True)
    else:
        st.info(
            f"📂 Image not yet generated: `{os.path.basename(path)}`  \n"
            "Run the Colab notebook first, then push the `visualizations/` folder to GitHub."
        )
...
def read_txt(path: str) -> str:
    if os.path.exists(path):
        with open(path) as f:
            return f.read()
    return f"⚠️ Report not found: {os.path.basename(path)}\nRun the notebook to generate this file."

def extract_kpi(txt: str, key: str) -> str:
...
.kpi-value { font-family: 'IBM Plex Mono', monospace; font-size: 22px; font-weight: 600; color: #58a6ff; }
.kpi-sub   { font-size: 10px; color: #8b949e; margin-top: 4px; }
.section-header {
    font-size: 19px; font-weight: 700; color: #f0f6fc;
    border-left: 4px solid #58a6ff; padding-left: 12px; margin: 28px 0 16px 0;
}
.story-box {
    background: #161b22; border: 1px solid #30363d; border-left: 4px solid #3fb950;
    border-radius: 6px; padding: 14px 18px; font-size: 14px;
    color: #c9d1d9; margin-bottom: 12px; font-style: italic;
}
.pipe-card { background: #161b22; border: 1px solid #21262d; border-radius: 8px; padding: 14px 10px; text-align: center; }
.pipe-icon  { font-size: 24px; }
.pipe-title { font-weight: 700; color: #f0f6fc; font-size: 13px; margin: 6px 0 3px; }
.pipe-desc  { font-size: 11px; color: #8b949e; }
</style>
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**Dataset** — Synthesized AAPL · GBM · 10 Years")
    st.markdown("**Models** — LSTM+Attn · GRU · Bi-LSTM · XGBoost")
    st.markdown("**XAI** — SHAP Gradient + TreeSHAP")
    st.markdown("---")
    st.markdown(
        "[![GitHub](https://img.shields.io/badge/GitHub-OmDadhe-181717?logo=github)]"
        "(https://github.com/OmDadhe/xai-financial-forecasting)  \n"
        "[![LinkedIn](https://img.shields.io/badge/LinkedIn-contactom-0077B5?logo=linkedin)]"
        "(https://linkedin.com/in/contactom)"
    )
...
if page == "🏠 Overview":
    ...
    for col, (icon, title, desc) in zip(cols, steps):   # 3-tuple unpacking matches 3-element tuples
        col.markdown(
    ...
    if not df_proc.empty:
        ...
        st.info("📂 Run the Colab notebook to generate `data/processed/AAPL_processed.csv`")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not df_proc.empty and "Close" in df_proc.columns:
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color="#58a6ff", width=1.2), name="Close",
            fill="tozeroy", fillcolor="rgba(88,166,255,0.07)",
        ))
        for col_name, color, dash in [("MA_20", "#f7931e", "dot"), ("MA_200", "#e74c3c", "solid")]:
            if col_name in df_proc.columns:
                fig.add_trace(go.Scatter(
                    x=df_proc["Date"], y=df_proc[col_name],
                    line=dict(color=color, width=1, dash=dash), name=col_name,
                ))
        fig.update_layout(
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    arch_df = pd.DataFrame([
        ("LSTM + Attention", "LSTM(128)",   "LSTM(64)",   "Temporal Attention", "(60, 12)", "GradientExplainer"),
        ("GRU",              "GRU(128)",    "GRU(64)",    "Update Gate",         "(60, 12)", "—"),
        ("Bi-LSTM",          "BiLSTM(128)", "BiLSTM(64)", "Bidirectional",       "(60, 12)", "—"),
        ("XGBoost",          "500 trees",   "depth=5",    "TreeSHAP (exact)",    "(720,)",   "TreeExplainer"),
    ], columns=["Model", "Layer 1", "Layer 2", "Key Feature", "Input Shape", "Explainer"])
...
feature subsets — satisfying Efficiency, Symmetry, and Dummy properties that simpler methods do not.
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        st.markdown("Raw dataset shape, NaN counts, dtypes, and descriptive statistics before any cleaning.")
    ...
    with tab2:
        ...
        st.markdown("Best model KPIs, full metrics table, top SHAP features, and dataset statistics.")
    ...
    for label, path in [
        ...
        if os.path.exists(path):
            ...
            st.caption(f"⚠️ {os.path.basename(path)} — not yet generated")
